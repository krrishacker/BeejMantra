...
const LoginPage: React.FC<Props> = ({ onSuccess, switchToSignup, currentLanguage, onLanguageChange, onGoHome }) => {
  ...
  const validate = () => {
    if (!/^\d{10}$/.test(phone)) return t('login.enterValidPhone');
      ...
      <div className="max-w-5xl mx-auto flex items-center justify-center min-h-[60vh]">
        ...
        <div className="text-center mb-4">
          <div className="mx-auto mb-2 w-16 h-16 rounded-full bg-green-100 flex items-center justify-center">
            {/* Simple farmer icon substitute */}
            <span className="text-2xl">🚜</span>
          </div>
          <h1 className="text-2xl font-bold text-green-700">{t('login.welcome')}</h1>
          <p className="text-sm text-gray-600 mt-1">{t('login.loginToContinue')}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="text-sm text-green-700 hover:underline" onClick={switchToSignup}>
